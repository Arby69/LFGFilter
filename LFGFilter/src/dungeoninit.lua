function LFGFilter:DefineDungeons()
	...
	self:DefineClassicDungeon("The Scarlet Monastery: Armory", "Tirisfal Glades", LFGFilter.Factions.BOTH, 29, 32, 39, 45, { "arm", "armory", "arms", "sm%W*arm%a*" }, { "arms%Wwar" })
	...
	self:DefineClassicDungeon("The Scarlet Monastery", "Tirisfal", LFGFilter.Factions.BOTH, 23, 26, 42, 45, { "sm", "monastery" })
	...
	self:DefineDungeon("classic", "Warsong Gulch", 10, "Ashenvale", LFGFilter.Factions.BOTH, 10, 10, 100, 100, 0, { "wsg", "warsong", "w[ar]*so?n?g" })
	...
	if LFGFilter.IsWOTLK then
		...
		self:DefineWotlkDungeon("Utgarde Keep", "Howling Fjord", 68, 70, 72, 75, { "uk", "u.*keep", "utgar[de]*%W[kep]+" }, { "shadow%a", "pinn%a*" }, { {477,1}, {489,2}, {1919,2}, {17213,3}, {18590,4} })
		self:DefineWotlkDungeon("The Nexus", "Borean Tundra", 70, 71, 73, 75, { "nex", "nexus", "nex%a*" }, {}, { {478,1}, {2037,2}, {490,2}, {2036,2}, {17283,3}, {18591,4} })
		self:DefineWotlkDungeon("Azjol-Nerub", "Dragonblight", 70, 72, 75, 77, { "azj", "azjo?l", "nerub" }, {}, { {480,1}, {491,2}, {1297,2}, {1860,2}, {1296,2}, {17285,3}, {18592,4} })
		self:DefineWotlkDungeon("Ahn'kahet: The Old Kingdom", "Dragonblight", 71, 73, 76, 78, { "ahnk", "tok", "ah%a*.?kah%a*", "old%Wkingd%a*", "ah?nkah?et" }, {}, { {481,1}, {492,2}, {2038,2}, {1862,2}, {2056,2}, {17291,3}, {18593,4} })
		self:DefineWotlkDungeon("Drak'Tharon Keep", "Grizzly Hills", 72, 74, 76, 78, { "dt", "dtk", "%a*k%W*tharon", "drak%W*th%a*" }, {}, { {482,1}, {493,2}, {2039,2}, {2057,2}, {2151,2}, {17292,3}, {18594,4} })
		self:DefineWotlkDungeon("Violet Hold", "Dalaran", 73, 75, 77, 79, { "vh", "vio%a*", "v%a*%W*hold" }, {}, { {483,1}, {494,2}, {1865,2}, {1816,2}, {2041,2}, {2153,2}, {17293,3}, {18595,4} })
		self:DefineWotlkDungeon("Gundrak", "", 75, 76, 78, 80, { "gd", "g%a*drak", "gun%Wdrak" }, {}, { {484,1}, {495,2}, {1864,2}, {2040,2}, {2152,2}, {2058,2}, {17295,3}, {18596,4} })
		self:DefineWotlkDungeon("Halls of Stone", "The Storm Peaks", 76, 77, 79, 80, { "hos", "hall%a*stone" }, { "seth%a*", "shatt%a*", "light%a*" }, { {485,1}, {496,2}, {2154,2}, {2155,2}, {1866,2}, {17297,3}, {18597,4} })
		self:DefineWotlkDungeon("Halls of Lightning", "The Storm Peaks", 78, 79, 80, 80, { "hol", "lightn[ing]*", "loken" }, { "seth%a*", "shatt%a*", "ston%a*" }, { {486,1}, {497,2}, {1834,2}, {1867,2}, {2042,2}, {17299,3}, {18598,4} })
		self:DefineWotlkDungeon("Utgarde Pinnacle", "Howling Fjord", 78, 79, 80, 80, { "up", "pin+ac[le]*", "ymiron", "utgar[de]*%Wpin+[acle]*" }, { "keep" }, { {488,1}, {499,2}, {2157,2}, {1873,2}, {2156,2}, {2043,2}, {17301,3}, {18600,4} })
		self:DefineWotlkDungeon("Oculus", "Borean Tundra", 78, 79, 80, 80, { "oculus", "oc+o?[lus]*" }, {}, { {487,1}, {498,2}, {2046,2}, {1871,2}, {1868,2}, {2044,2}, {2045,2}, {17300,3}, {18599,4} })
		self:DefineWotlkDungeon("Culling of Stratholme", "Tanaris", 78, 79, 80, 80, { "cos", "culling", "cot.*4", "strat", "strath%a*", "starth%a*", "straht%a*", "%a*ganis" }, { "baron" }, { {479,1}, {500,2}, {1817,2}, {1872,2}, {17302,3}, {18601,4} })
		self:DefineWotlkDungeon("Trial of the Champion", "Icecrown", 78, 79, 80, 80, { "toc", "trial", "trail", "tr[ial]*[%wofthe]+cha[mpion]*" }, {}, { {3802,2}, {3803,2}, {3804,2}, {4298,2}, {18677,4} })
		...
		self:DefineWotlkRaid("Obsidian Sanctum", "Dragonblight", { "obsi", "os", "sanctum", "obsi%dd?", "obsi.%d", "obsi10s+", "obsi25s+", "sar?t[harion]*", "s[123]d" }, { "ms", "ruby" })
		self:DefineWotlkRaid("Vault of Archavon", "Wintergrasp", { "voa", "archa[von]*", "ema[lon]*", "kora[lon]*" })
		self:DefineWotlkRaid("Ulduar", "The Storm Peaks", { "ulduar", "yogg.*", "uldu", "u[lduar]*10%a*", "u[lduar]*25%a*" })
		self:DefineWotlkRaid("Trial of the Crusader", "Icecrown", { "tocr", "togc", "gr[and]+%W*cru[sader]+", "tog?c10%a*", "tog?c25%a*" })
		...
		-- these Raids were replaced by new versions in WotLK
		self:DefineClassicRaid("Onyxia's Lair", 40, "Dustwallow Marsh", 55, 58, { "ony", "onyxia" }, { "pre" }) -- replaced by 80s version in WotLK phase 3
